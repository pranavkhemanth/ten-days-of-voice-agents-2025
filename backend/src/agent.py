...
@dataclass
class FraudCase:
    """Represents a fraud case record."""
    userName: str
...
def seed_database():
    ...
    if cur.fetchone()[0] == 0:
        sample_data = [
            (
                "John", "21212", "4242",
                "ABC Industry", "$690.00", "12:30 AM EST", "ebay.com",
                "pending_review", "Automated flag: High value transaction."
            ),
            (
                "Christeena", "99887", "1199",
        ...
        conn.commit()
        logger.info(f"SQLite DB seeded at {DB_FILE}")
    conn.close()
...
@dataclass
class Userdata:
    """Stores user-specific data during a session."""
...
async def lookup_customer(
    ...
    """
    Lookup a customer in the SQLite database by name.
    Returns the customer's fraud case details if found.
    """
...
async def resolve_fraud_case(
    ...
    """
    Resolve a fraud case by updating its status in the database.
    Marks the case as either confirmed_safe or confirmed_fraud.
    """
...
class FraudAgent(Agent):
    """
    Agent that handles fraud detection and resolution calls.
    Follows a strict security protocol to verify identity and resolve cases.
    """
    def __init__(self):
        super().__init__(
            instructions="""
            You  are a Fraud Detection Automated Bot at Heritage Bank.
            Follow this security protocol:
            1. Greet the user and ask for their first name.
            2. Look up the customer using lookup_customer(name).
            3. Ask for their Security Identifier.
            4. If correct, continue. If incorrect, end the call politely.
            5. Explain the suspicious transaction.
...
def prewarm(proc: JobProcess):
    """Preload resources required for the agent session."""
    proc.userdata["vad"] = silero.VAD.load()

async def entrypoint(ctx: JobContext):
    """Initialize and start the fraud alert session."""
